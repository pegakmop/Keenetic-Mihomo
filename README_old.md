...
Пример конфига (при вставке в nano может быть проблема с русским шрифтом, я это обошел через редактор из MC):

* За правила маршрутизации благодарность уважаемому Легизу: https://github.com/legiz-ru/mihomo-rule-sets

* Не забудьте вставить свои данные в раздел "# --- ВАШИ ПРОКСИ-СЕРВЕРЫ ---"

* В моем примере remnanoda.domain.com это и адрес ноды с прокси и адрес селфстила (сайта странички)
...
Нужно будет ввести наш адрес:порт и придумать пароль
...
* Временный, потому что после перезагрузки не сохраняется. Как сохранить - читаем ниже
...
* Это не проксирует UDP — только TCP.
* Если кто-то внутри сети начнёт использовать нестандартные способы обхода (например, QUIC или DNS over HTTPS/3), это не перехватится.
* DNS-запросы тоже нужно заворачивать через ядро, иначе возможны DNS leaks.

# РЕЗЮМЕ

Данный способ подойдет для обычного веб серфинга домашних устройств. Если нужно "заворачивать" в ядро все устройства роутера, не создавая статичные адреса для некоторых, то нужно сделать скрипт для перенаправления всех устройств на нужный порт.

Для более полноценной работы ядра Михомо, в том числе и работы с TCP и UDP, желательно включить режим TUN. На моем роутере Kenetic Hopper SE он есть, но пока не удалось полноценно перенаправить трафик в TUN.

* Более интересный способ запуска ядра Mihomo на роутерах Keenetic вы можете найти у другого автора вот здесь: https://www.notion.so/XKeen-mihomo-b60985fca7054a4f9dc9936e7611be4f
